...
//p:- edge tangent flow with more weightage to current pixel
void ETF::set(imatrix& image) 
{
	...
	max_grad = -1.;
	//assiging p[i][j] valuse from the neghbouring pixels to i, j
	for (i = 1; i < Nr - 1; i++) { 
		for (j = 1; j < Nc - 1; j++) {
